...
async function checkServiceStatus() {
    ...
    try {
        ...
        if (adminOk && coreOk) {
            statusEl.innerHTML = '<span style="color: #48bb78;">NORMAL</span>';
        } else {
            const issues = [];
            if (!adminOk) {
                const error = adminHealth.error || 'Unknown';
                issues.push(`Admin (${error})`);
            }
            if (!coreOk) {
                const error = coreHealth.error || 'Unknown';
                issues.push(`Core (${error})`);
            }
            statusEl.innerHTML = `<span style="color: #ed8936;">WARN: ${issues.join(', ')}</span>`;
        }
    } catch (error) {
        console.error('Service status check error:', error);
        statusEl.innerHTML = '<span style="color: #f56565;">ERROR: ' + error.message + '</span>';
    }
}
...
async function loadMarketCount() {
    const el = document.getElementById('market-count');
    if (!el) return;
    try {
        const resp = await fetch(`${CONFIG.CORE_BASE_URL}/api/market-data`);
        const result = await resp.json();
        if (result.code === 0 && result.data) {
            el.textContent = Object.keys(result.data).length;
        } else {
            el.textContent = '0';
        }
    } catch (e) {
