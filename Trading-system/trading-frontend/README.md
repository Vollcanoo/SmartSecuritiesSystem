...
├── user-analysis.html  # 用户交易分析页面（统计 + 可视化）
...
### 4.6 用户分析（user-analysis.html）

- 输入 **用户ID(uid)** 与时间范围（start/end），调用 Admin 的分析接口并展示统计结果。
- 展示指标：订单总数、成交订单数、撤单订单数、成交总金额、成交均价。
- 提供简单可视化：成交 vs 撤单占比条形图。
