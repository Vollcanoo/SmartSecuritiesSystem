#!/usr/bin/env python3
"""
Visualize throughput benchmark JSON results without third-party dependencies.

Input format:
- JSON output generated by throughput-tests/run_throughput.py
    ...
    for item in data.get("results", []):
        ...
        rows.append(
            RunRow(
                ...
                p999_ms=as_num(item.get("p999_ms", item.get("p99_ms", 0))),
                max_ms=as_num(item.get("max_ms", item.get("p99_ms", 0))),
                std_ms=as_num(item.get("std_ms", 0)),
                reject_codes=reject_codes,
            )
        )
    rows.sort(key=lambda r: (r.concurrency, r.run_name))
    return Dataset(
        id=dataset_id,
        file_name=path.name,
        target=target,
...
def svg_stacked_outcome_chart(
    title: str,
    categories: List[str],
    values: List[Tuple[int, int, int]],  # ok, fail, biz
    width: int = 920,
    height: int = 420,
) -> str:
    left = 70
...
"""
    ...
    for ds in datasets:
        ...
        for r in rows:
            ...
            parts.append(f"<td>{r.p999_ms:.2f}</td>")
        ...
        rps_svg = svg_line_chart(
            title=f"{ds.id} - Throughput vs Concurrency",
            x_label="Concurrency",
            y_label="RPS",
            series=[
                ("rps", [(float(r.concurrency), float(r.rps)) for r in rows]),
                ("effective_rps", [(float(r.concurrency), float(r.effective_rps)) for r in rows]),
            ],
        )
        lat_svg = svg_line_chart(
            title=f"{ds.id} - Latency vs Concurrency",
            x_label="Concurrency",
            y_label="Latency (ms)",
            series=[
                ("p50_ms", [(float(r.concurrency), float(r.p50_ms)) for r in rows]),
                ("p90_ms", [(float(r.concurrency), float(r.p90_ms)) for r in rows]),
                ("p95_ms", [(float(r.concurrency), float(r.p95_ms)) for r in rows]),
                ("p99_ms", [(float(r.concurrency), float(r.p99_ms)) for r in rows]),
                ("p999_ms", [(float(r.concurrency), float(r.p999_ms)) for r in rows]),
            ],
        )
        quality_svg = svg_line_chart(
            title=f"{ds.id} - Quality Rates vs Concurrency",
            x_label="Concurrency",
            y_label="Rate (%)",
            series=[
                ("success_rate_pct", [(float(r.concurrency), float(r.success_rate_pct)) for r in rows]),
                ("fail_rate_pct", [(float(r.concurrency), float(r.fail_rate_pct)) for r in rows]),
                ("biz_reject_rate_pct", [(float(r.concurrency), float(r.biz_reject_rate_pct)) for r in rows]),
            ],
        )
...
def main() -> int:
    ...
    for ds in datasets:
        arr = sorted(ds.rows, key=lambda r: r.concurrency)
        rps_series.append((ds.id, [(float(r.concurrency), float(r.rps)) for r in arr]))
        effective_rps_series.append((ds.id, [(float(r.concurrency), float(r.effective_rps)) for r in arr]))
        p95_series.append((ds.id, [(float(r.concurrency), float(r.p95_ms)) for r in arr]))
        fail_rate_series.append((ds.id, [(float(r.concurrency), float(r.fail_rate_pct)) for r in arr]))
        biz_reject_rate_series.append((ds.id, [(float(r.concurrency), float(r.biz_reject_rate_pct)) for r in arr]))
        all_rows.extend(arr)

    global_rps_svg = svg_line_chart(
        title="All Datasets - RPS vs Concurrency",
        x_label="Concurrency",
        y_label="RPS",
        series=rps_series,
    )
    global_effective_rps_svg = svg_line_chart(
        title="All Datasets - Effective RPS vs Concurrency",
        x_label="Concurrency",
        y_label="Effective RPS",
        series=effective_rps_series,
    )
    global_p95_svg = svg_line_chart(
        title="All Datasets - P95 Latency vs Concurrency",
        x_label="Concurrency",
        y_label="P95 Latency (ms)",
        series=p95_series,
    )
    global_fail_rate_svg = svg_line_chart(
        title="All Datasets - Fail Rate (%) vs Concurrency",
        x_label="Concurrency",
        y_label="Fail Rate (%)",
        series=fail_rate_series,
    )
    global_biz_reject_rate_svg = svg_line_chart(
        title="All Datasets - Biz Reject Rate (%) vs Concurrency",
        x_label="Concurrency",
