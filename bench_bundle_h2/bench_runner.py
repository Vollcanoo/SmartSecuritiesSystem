...
def main() -> int:
    ...
    default_plan = "5000x20,10000x50,12000x80" if args.target == "core" else "2000x20,5000x50,8000x80"
    ...
    for i, (total, conc) in enumerate(plan, start=1):
        name = f"run{i}_{total}x{conc}"
        print(f"[info] running {name} ...")
        if args.target == "core":
            case = run_http_case(name, args.core_url, payload_json, total, conc, timeout_sec)
        else:
            case = run_tcp_case(name, args.gateway_host, args.gateway_port, payload_json, total, conc, timeout_sec)
        results.append(case)
